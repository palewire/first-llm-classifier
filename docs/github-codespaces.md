...
It will take a minute or two to build your environment. When it's ready, you'll see a full Visual Studio Code editor in your browser with everything pre-installed. A terminal will be open at the bottom of the screen.
...
Click "File" in the menu bar and select "New File..." from the dropdown. In the browser based version of Codespaces, the "File" menu is located in the upper left corner of the screen, where you need to click on the icon with three horizontal lines.

When prompted to choose a file type, select "Jupyter Notebook."

You will see a prompt in the upper right corner that says "Select Kernel." A popup will appear. Select "Python Environments..." and then choose the option that includes `.venv`. It probably has a name that includes "first-llm-classifier."
...
If so, congratulations. You're all set up and ready to move on to writing code. Jump into our chapter on [prompting with Python](prompting-with-python.md) to get started.
